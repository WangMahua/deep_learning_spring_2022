...
public:
	...
	state select_best_move(const board& b) const {
		...
		state* best = after;
		for (state* move = after; move != after + 4; move++) { //go through up, right, down, left
			if (move->assign(b)) {
	...
	void load(const std::string& path) {
		...
		if (in.is_open()) {
			...
			debug << "Can not open bin file" << std::endl;
		}
	}
...
void save_score(size_t episode, int score){
	std::ofstream file;
	file.open("./record_score/score.txt",std::ios::out  | std::ios::app);
